...
func (w *response) Stream() Stream {
	return &h1Stream{w.conn.rwc}
}
...
func (c *conn) readRequest(ctx context.Context) (w *response, err error) {
	...
	w = &response{
		...
		wants10KeepAlive: requestWantsHttp10KeepAlive(req),
		wantsClose:       requestWantsClose(req),
	}
	if isH2Upgrade {
		w.closeAfterReply = true
	}
...
func (w *response) Header() http.Header {
	...
	w.calledHeader = true
	return http.Header(w.handlerHeader)
}
...
func relevantCaller() runtime.Frame {
	...
	for {
		...
		if !strings.HasPrefix(frame.Function, "github.com/puernya/go-http.") {
			return frame
		}
		if !more {
			break
...
func (w *response) WriteHeader(code int) {
	...
	if cls, exist := w.handlerHeader["Content-Length"]; exist && len(cls) > 0 && cls[0] != "" {
		v, err := strconv.ParseInt(cls[0], 10, 64)
		if err == nil && v >= 0 {
			w.contentLength = v
		} else {
			w.conn.server.logf("http: invalid Content-Length of %q", cls[0])
...
func (cw *chunkWriter) writeHeader(p []byte) {
	...
	if w.handlerDone.Load() && !trailers && !hasTE && bodyAllowedForStatus(w.status) && !headerHas(header, "Content-Length") && (!isHEAD || len(p) > 0) {
	...
	if w.wants10KeepAlive && keepAlivesEnabled {
		sentLength := getFromHeader(header, "Content-Length") != ""
		if sentLength && getFromHeader(header, "Connection") == "keep-alive" {
	...
	if getFromHeader(header, "Connection") == "close" || !keepAlivesEnabled {
		w.closeAfterReply = true
	}
	...
	delConnectionHeader := w.closeAfterReply &&
		(!keepAlivesEnabled || !hasToken(getFromHeader(cw.header, "Connection"), "close")) &&
...
func (w *response) bodyAllowed() bool {
	if !w.wroteHeader {
		panic("github.com/puernya/go-http: bodyAllowed called before the header was written")
	}
...
func (c *conn) serve(ctx context.Context) {
	...
	for {
		...
		} else if getFromHeader(req.Header, "Expect") != "" {
...
func (h unencryptedHTTP2Request) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	if req.Body == nil {
...
func (w *response) Hijack() (rwc net.Conn, buf *bufio.ReadWriter, err error) {
	if w.handlerDone.Load() {
		panic("github.com/puernya/go-http: Hijack called after ServeHTTP finished")
	}
	w.disableWriteContinue()
	if w.wroteHeader {
		w.cw.flush()
...
func (w *response) CloseNotify() <-chan bool {
	...
	if w.handlerDone.Load() {
		panic("github.com/puernya/go-http: CloseNotify called after ServeHTTP finished")
	}
...
type Server struct {
	...
	TLSNextProto map[string]func(*Server, TLSConn, http.Handler)
