...
import (
	...
	"github.com/puernya/go-http/internal/httpcommon"
	"github.com/puernya/go-http/internal/httpsfv"
...
func (p *http2clientConnPool) closeConnections() {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func http2configFromTransport(h2 *http2Transport) http2http2Config {
	conf := http2http2Config{
		...
		SendPingTimeout:             h2.ReadIdleTimeout,
		PingTimeout:                 h2.PingTimeout,
		IdleTimeout:                 h2.IdleConnTimeout,
		WriteByteTimeout:            h2.WriteByteTimeout,
	}
...
func http2fillNetHTTPConfig(conf *http2http2Config, h2 *HTTP2Config) {
	...
	if h2.IdleTimeout != 0 {
		conf.IdleTimeout = h2.IdleTimeout
	}
...
func http2ConfigureServer(s *Server, conf *http2Server) error {
	...
	if s.TLSNextProto == nil {
		s.TLSNextProto = map[string]func(*Server, TLSConn, http.Handler){}
	}
	protoHandler := func(hs *Server, c net.Conn, h http.Handler, sawClientPreface bool) {
	...
	s.TLSNextProto[http2NextProtoTLS] = func(hs *Server, c TLSConn, h http.Handler) {
		protoHandler(hs, c, h, false)
	}
	// The "unencrypted_http2" TLSNextProto key is used to pass off non-TLS HTTP/2 conns.
	//
	// A connection passed in this method has already had the HTTP/2 preface read from it.
	s.TLSNextProto[http2nextProtoUnencryptedHTTP2] = func(hs *Server, c TLSConn, h http.Handler) {
...
func (sc *http2serverConn) newWriterAndRequestNoBody(st *http2stream, rp httpcommon.ServerRequestParam) (*http2responseWriter, *http.Request, error) {
	sc.serveG.check()
...
func (w *http2responseWriter) Header() http.Header {
	...
	return http.Header(rws.handlerHeader)
}
...
func (w *http2responseWriter) Stream() Stream {
	return &h2Stream{w.rws}
}
...
func (s *h2Stream) Read(p []byte) (int, error) {
	if !s.rws.wroteHeader {
		panic("cannot use stream before write response header")
	}

	if s.rws.handlerDone {
		s.rws.promoteUndeclaredTrailers()
	}

	return s.rws.req.Body.Read(p)
}
...
func (s *h2Stream) Write(p []byte) (int, error) {
	...
	if len(p) > 0 {
		// only send a 0 byte DATA frame if we're ending the stream.
		if err := s.rws.conn.writeDataFromHandler(s.rws.stream, p, false); err != nil {
			return 0, err
		}
	}
	return len(p), nil
...
type http2Transport struct {
	...
	DialTLSContext func(ctx context.Context, network, addr string, cfg *tls.Config) (TLSConn, error)
	...
	DialTLS func(network, addr string, cfg *tls.Config) (TLSConn, error)
...
func http2configureTransports(t1 *Transport) (*http2Transport, error) {
	...
	if t1.TLSNextProto == nil {
		t1.TLSNextProto = make(map[string]func(string, TLSConn) http.RoundTripper)
	}
	t1.TLSNextProto[http2NextProtoTLS] = func(authority string, c TLSConn) http.RoundTripper {
		return upgradeFn("https", authority, c)
	}
	// The "unencrypted_http2" TLSNextProto key is used to pass off non-TLS HTTP/2 conns.
	t1.TLSNextProto[http2nextProtoUnencryptedHTTP2] = func(authority string, c TLSConn) http.RoundTripper {
		nc, err := http2unencryptedNetConnFromTLSConn(c)
		if err != nil {
			go c.Close()
			return http2erringRoundTripper{err}
...
func (t *http2unencryptedTransport) RoundTrip(req *http.Request) (*http.Response, error) {
...
func (t *http2Transport) CloseConnections() {
...
func (t *http2Transport) dialTLS(ctx context.Context, network, addr string, tlsCfg *tls.Config) (TLSConn, error) {
...
// A TLSConn.Close can hang for a long time if the peer is unresponsive.
...
func (cc *http2ClientConn) roundTrip(req *http.Request, streamf func(*http2clientStream)) (*http.Response, error) {
	ctx := req.Context()
...
func (cs *http2clientStream) doRequest(req *http.Request, streamf func(*http2clientStream)) {
	err := cs.writeRequest(req, streamf)
	cs.cleanupWriteRequest(err)
}

var http2errExtendedConnectNotSupported = errors.New("http: extended connect not supported by peer")
...
func (cs *http2clientStream) writeRequest(req *http.Request, streamf func(*http2clientStream)) (err error) {
	cc := cs.cc
	ctx := cs.ctx
...
func http2encodeRequestHeaders(req *http.Request, addGzipHeader bool, peerMaxHeaderListSize uint64, headerf func(name, value string)) (httpcommon.EncodeHeadersResult, error) {
...
func (rt http2noDialH2RoundTripper) NewClientConn(conn net.Conn, internalStateHook func()) (http.RoundTripper, error) {
...
func (cc http2netHTTPClientConn) RoundTrip(req *http.Request) (*http.Response, error) {
