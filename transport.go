...
// Transport is an implementation of [net/http.RoundTripper] that supports HTTP,
// HTTPS and unencrypted HTTP/2.
//
...
type Transport struct {
	...
	DialTLS func(network, addr string) (TLSConn, error)
...
func (t *Transport) Clone() *Transport {
	...
	if !t.tlsNextProtoWasNil {
		...
		if npm == nil {
			npm = make(map[string]func(authority string, c TLSConn) http.RoundTripper)
		}
		t2.TLSNextProto = npm
	}
	return t2
...
func (t *Transport) roundTrip(req *http.Request) (_ *http.Response, err error) {
	...
	if isHTTP {
		...
		if err := validateHeaders(req.Header); err != "" {
			closeRequestBody(req)
			return nil, fmt.Errorf("github.com/puernya/go-http: invalid header %s", err)
		}

		// Validate the outgoing trailers too.
		if err := validateHeaders(req.Trailer); err != "" {
			closeRequestBody(req)
			return nil, fmt.Errorf("github.com/puernya/go-http: invalid trailer %s", err)
		}
	}
	...
	if req.Method != "" && !validMethod(req.Method) {
		closeRequestBody(req)
		return nil, fmt.Errorf("github.com/puernya/go-http: invalid method %q", req.Method)
	}
	if req.URL.Host == "" {
		closeRequestBody(req)
...
var errCannotRewind = errors.New("http: cannot rewind body after connection loss")
...
func rewindBody(req *http.Request) (rewound *http.Request, err error) {
	if req.Body == nil || req.Body == http.NoBody || (!req.Body.(*readTrackingBody).didRead && !req.Body.(*readTrackingBody).didClose.Load()) {
...
func (pc *persistConn) shouldRetryRequest(req *http.Request, err error) bool {
	...
	if _, ok := err.(nothingWrittenError); ok {
		...
		return requestOutgoingLength(req) == 0 || req.GetBody != nil
	}
...
// in use.
func (t *Transport) CloseConnections() {
	t.nextProtoOnce.Do(t.onceSetNextProtoDefaults)
	t.idleMu.Lock()
	m := t.idleConn
	t.idleConn = nil
...
func (e transportReadFromServerError) Error() string {
	return fmt.Sprintf("github.com/puernya/go-http: Transport failed to read from server: %v", e.err)
}
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	if t.DialContext != nil {
		...
		if c == nil && err == nil {
			err = errors.New("http: Transport.DialContext hook returned (nil, nil)")
		}
		return c, err
	}
	if t.Dial != nil {
...
func (w *wantConn) tryDeliver(pc *persistConn, err error, idleAt time.Time) bool {
	...
	if (pc == nil) == (err == nil) {
		panic("github.com/puernya/go-http: internal error: misuse of tryDeliver")
	}
	w.ctx = nil
	w.done = true
...
func (t *Transport) decConnsPerHost(key connectMethodKey) {
	...
	if n == 0 {
		...
		panic("github.com/puernya/go-http: internal error: connCount underflow")
	}
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod, isClientConn bool, internalStateHook func()) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.hasCustomTLSDialer() {
		tc, err := t.customDialTLS(ctx, "tcp", cm.addr())
		if err != nil {
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	if pc.isBroken() {
		...
		return fmt.Errorf("github.com/puernya/go-http: HTTP/1.x transport connection broken: %w", err)
	}
	return err
}
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err != nil {
			if pc.readLimit <= 0 {
				err = fmt.Errorf("github.com/puernya/go-http: server response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
			}
		...
		bodyWritable := isResponseBodyWritable(resp)
...
func (b *readWriteCloserBody) CloseWrite() error {
	...
	return fmt.Errorf("CloseWrite: %w", http.ErrNotSupported)
}
...
var errTimeout error = &timeoutError{"github.com/puernya/go-http: timeout awaiting response headers"}

// errRequestCanceled is set to be identical to the one from h2 to facilitate
// testing.
var (
	errRequestCanceled     = http2errRequestCanceled
	errRequestCanceledConn = errors.New("http: request canceled while waiting for connection") // TODO: unify?
)
...
var errRequestDone = errors.New("http: request completed")
...
var (
	...
	errConcurrentReadOnResBody = errors.New("http: concurrent read on response body")
)
