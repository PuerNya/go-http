...
func (t *Transport) NewClientConn(ctx context.Context, scheme, address string) (*ClientConn, error) {
	...
	case "http", "https":
	default:
		return nil, fmt.Errorf("github.com/puernya/go-http: invalid scheme %q", scheme)
	}
...
func validateClientConnRequest(req *http.Request) error {
	if req.URL == nil {
...
func (cc http1ClientConn) RoundTrip(req *http.Request) (*http.Response, error) {
	ctx := req.Context()
